...
options( mc.cores = 3 )
path = 'C:/Users/8936381/Documents/rstan/ts/stan_ts_model.stan'
...
colnames(data) = c('média', 'mediana', 'sd', 'assimetria', 'curtose', 'min', 'max')
data = round(data, 3)
data
# Running stan code
T = length(y)
...
df.1 = data.frame(x=1:T, est = h_hat, 
...
df.2 = data.frame(x=1:T, est = l_hat, 
                  lim_inf = lim_inf, lim_sup = lim_sup)
h = ggplot( df.2[1:300, ] ) 
